...
MonotonicFunctions::DiagonalDS MonotonicFunctions::diagonals(vector<Point3f> &curve,vector<int> &partition,float sw,float aw,float iw, int index) {
    DiagonalDS result;
    if(partition.size()==2){
        result.setDiag(partition[0],partition[1],vcg::Distance(curve[partition[0]],curve[partition[1]]),0,0);
        return result;
    }
...
vector<MonotonicFunctions::Polygon> MonotonicFunctions::PrePatch2D::splitCurve(MonotonicFunctions::Polygon &polygon, float limitAngle, int maxPartition,float sw,float aw,float iw){
    ...
    if(diag.size==-1){ // impossible to make a diag in this polygon
       while(polygon.sides.size()> maxPartition)
            polygon.collapseOneSide();
       result.push_back(polygon);
       return result;
    }
   ...
   */
    ...
    //a.saveAsPLY("a-result");
    //b.saveAsPLY("b-result");
    result.push_back(a);
    result.push_back(b);
    return result;
}
