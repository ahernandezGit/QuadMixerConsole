...
namespace MonotonicFunctions {
    ...
    struct Polygon{
        ...
        void Set(Polygon &other){
            sides.clear();
            numberID=other.numberID;
            parent=other.parent;
            adjacency.clear();
            for(size_t i=0;i<other.sides.size();i++){
               PolygonSide temp;
               temp.vertices=other.sides[i].vertices;
               temp.flag=other.sides[i].flag;
               temp.id=other.sides[i].id;
               temp.index_3D=other.sides[i].index_3D;
               temp.size=other.sides[i].size;
        ...
        void collapseOneSide(){
            struct angleind{
                double angle;
                size_t ind;
                bool operator > (const angleind& str) const
                {
            ...
            // analyzing if the endpoints of sides are higher than 150 degrees
            size_t ns=sides.size();
            if(ns==2) // we can't compacting more
                return;

            saveAsPLY("test");
            vector<angleind> candidates;
            for(size_t i=0;i<ns;i++){
               size_t nsi=sides[i].vertices.size();
               assert(nsi>1);
               Point3f a=sides[i].vertices[nsi-2]-sides[i].vertices[nsi-1];
               Point3f b=sides[(i+1)%ns].vertices[1]-sides[(i+1)%ns].vertices[0];
               double angle=double(vcg::Angle(a,b));
               angleind ai={angle,i};
               candidates.push_back(ai);
            }

            vector<angleind> copy=candidates;
            std::sort(copy.begin(),copy.end(),greater<angleind>());

            // creating new polygon

            //first side
            size_t start=copy[0].ind;
            size_t next=(start+1)%ns;

            vector<Point3f> temp_side=sides[start].vertices;
            vector<Point3f> aux_side=sides[next].vertices;
            aux_side.erase(aux_side.begin());
            temp_side.insert(temp_side.end(),aux_side.begin(),aux_side.end());
            PolygonSide tss(temp_side,S_UNDEFINED,-1,-1);
            vector<PolygonSide> temp_poly;
            temp_poly.push_back(tss);

            //other sides
            for(size_t i=2;i<ns;i++){
                size_t thisside=(start+i)%ns;
                vector<Point3f> cps=sides[thisside].vertices;
                PolygonSide ts(cps,S_UNDEFINED,-1,-1);
                temp_poly.push_back(ts);
            }
            Polygon npoly(temp_poly,numberID);
            npoly.saveAsPLY("testafter");
            Set(npoly);
        }
        void compacting(){
            saveAsPLY("test");
            vector<Point3f> poly=get_as_points();
            set<size_t> marked;
            size_t np=poly.size();
            if(np<3) // we can't compacting more
                return;
            double angthr=5*M_PI/6;
            while(marked.size()<2){
                for(size_t i=0;i<np;i++){
                    Point3f a=poly[(i-1+np)%np];
                    Point3f b=poly[i];
                    Point3f c=poly[(i+1)%np];
                    a=a-b;
                    b=c-b;
                    double angle=double(vcg::Angle(a,b));
                    if(angle<angthr){
            ...
            vector<size_t> endpoints(marked.begin(),marked.end());
            std::sort(endpoints.begin(),endpoints.end());

            // creating new polygon
            size_t start=endpoints[0];
            vector<Point3f> temp_side;
            vector<PolygonSide> temp_poly;
            for(size_t i=0;i<=np;i++){
                size_t index=(i+start)%np;
                if(marked.find(index)!=marked.end()){
                    if(!temp_side.empty()){
                        temp_side.push_back(poly[index]);
                        PolygonSide tss(temp_side,S_UNDEFINED,-1,-1);
                        temp_poly.push_back(tss);
                        temp_side.clear();
                    }
