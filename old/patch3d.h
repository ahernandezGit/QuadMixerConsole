...
public:
    ...
    */
    void computeBorders(bool computeparameterization=true){
        ...
        //Creating a reference parameterization of the patch. This is a mapping between the surface patch and the unit square.
        //parameterization.initialize(*tmesh,*quadmesh,numberpartitionQuadMesh);
        ...
        for(int i=0;i<numberConnectedComponents;i++){
            ...
            //Third, we go back to the begining of the quad patch side or to the first cut with a intersection curve.
             ...
             do{
                  ...
                  pbegin.FlipV();
                  isNonIntersectionCurveEdge=(hMeshNumber[pbegin.F()]==hMeshNumber[pbegin.FFlip()]);
                  int qibefore=convert34(hCorrespondence[pbefore.F()]);
                  int qibegin=convert34(hCorrespondence[pbegin.F()]);
                  hasSameQuadFace=(qibefore==qibegin);
             }while(!isNonIntersectionCurveEdge || !hasSameQuadFace);
             ...
             do{
                 ...
                 if(isNonIntersectionCurveEdge && isNonIntersectionCurveEdgeBefore){
                     ...
                     hasSameQuadFace=false;

                 //finishing one side originated from a quad side
                 if(isNonIntersectionCurveEdge && hasSameQuadFace && isNonIntersectionCurveEdgeBefore){
                     Side<TriMeshType> tempSide;
             ...
             if(numbersides>2){
                 ...
                 zerozero=tri::Index(*booleanmesh,sidesCB.sidesT[0].origin.VFlip());
                 oneone=tri::Index(*booleanmesh,sidesCB.sidesT[middle].origin.V());
             }
             ...
             stringcomponent+="/component0"+std::to_string(i)+".ply";
     ...
     * */
    template<class CurveClass>
    void setBoundaries2D(CurveClass &intersection_curve){
